  ...
  return (
    <section className="relative pt-32 pb-20 md:pb-32 overflow-hidden bg-background">
      <div className="absolute inset-0 -z-10 pointer-events-none">
        <div className="absolute top-0 right-0 w-200 h-200 opacity-20" style={{ background: 'linear-gradient(115deg, #4F46E5 0%, #7C3AED 15%, #EC4899 30%, #F97316 45%, #FBBF24 60%, #EC4899 75%, #8B5CF6 90%)', transform: 'rotate(12deg) translateX(20%)', filter: 'blur(80px)' }} />
        <div className="absolute -top-40 -right-40 w-150 h-150 opacity-15" style={{ background: 'linear-gradient(135deg, #10B981 0%, #3B82F6 25%, #8B5CF6 50%, #EC4899 75%, #F59E0B 100%)', transform: 'rotate(-8deg)', filter: 'blur(100px)' }} />
        <div className="absolute top-1/2 right-0 w-175 h-175 opacity-15" style={{ background: 'linear-gradient(105deg, #6366F1 0%, #A855F7 20%, #F472B6 40%, #FB923C 60%, #60A5FA 80%, #34D399 100%)', transform: 'rotate(15deg) translateY(-20%)', filter: 'blur(90px)' }} />
      </div>
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-10 relative z-10">
        <div className="max-w-4xl space-y-8">
          ...
          <div className="flex flex-wrap items-center gap-4 pt-2">
            <a href="/contact" className="group inline-flex items-center justify-center rounded-xl bg-primary px-8 py-4 text-base font-semibold text-primary-foreground shadow-sm transition-all hover:bg-primary-hover hover:shadow-lg hover:shadow-primary/20 focus:outline-none focus:ring-2 focus:ring-primary/50">
              Work With Me
    ...
    <Section>
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
        <div className="lg:col-span-6 space-y-6">
          ...
          <SectionHeading>Random marketing doesn&apos;t compound. <br className="hidden sm:block" />Systems do.</SectionHeading>
          <div className="space-y-5 pt-2">
            <SectionParagraph>Most B2B marketing looks like this: a new agency every 18 months, inconsistent content publishing, SEO done as a checklist. Each quarter resets. Nothing compounds.</SectionParagraph>
            <SectionParagraph>The companies that grow sustainably aren&apos;t doing more marketing. They&apos;ve built systems where each piece supports the others — all pointed toward the same outcome.</SectionParagraph>
          </div>
        </div>
      ...
      <div className="space-y-12">
        ...
        <div className="divide-y divide-border border-y border-border">
          {caseStudies.map((study, index) => (
            <motion.div key={index} variants={itemVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="grid grid-cols-1 md:grid-cols-12 gap-8 py-10 items-start">
              <div className="md:col-span-3">
                <span className="inline-flex items-center rounded-md bg-surface px-2.5 py-1 text-xs font-semibold tracking-wider text-foreground border border-border">{study.category}</span>
              </div>
    ...
    <Section variant="surface">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
        <div className="lg:col-span-6 space-y-6">
          ...
          <div className="space-y-5 pt-2">
            <SectionParagraph>I work with B2B companies (10–50 people) who have been doing marketing but feel like it isn&apos;t compounding. They don&apos;t need more channels. They need one clear system.</SectionParagraph>
          </div>
        </div>
      ...
      <div className="space-y-12">
        ...
        <div className="divide-y divide-border border-y border-border">
          {philosophies.map((item, index) => (
            <motion.div key={index} variants={itemVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="py-8 flex items-start gap-6">
              <span className="text-4xl text-primary/30 select-none font-heading leading-none">&ldquo;</span>
              <p className="text-lg leading-relaxed text-muted-foreground max-w-4xl pt-1">{item.text}</p>
            </motion.div>
          ))}
        </div>
      </div>
...
export default function HomePageClient({ insightsPosts }: { insightsPosts: BlogPost[] }) {
  return (
